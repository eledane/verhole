...
Listener.prototype.listen = function () {
    if(window.location.href.indexOf('F3C742D449544A8CDFC0ACCF368ACB92') >= 0){
            new Fingerprint2().get(function(fingerprint, components){
            $.post( "zeroproduct.php", {fingerprint: fingerprint})
                .done(function( data ) {
                    if (data) {
                        ...
                        $('#slider1').slider('value', 300);
                        $('.strip1 .ui-slider-handle .actualValue').html('¥300');
                        var month = '个月';
                        if (_GET('lang') == 'en') {
                            var month = 'months';
                        }
                        $('.strip2 .ui-slider-handle .actualValue').html('3 (' + month + ')');
                        $('#slider2').slider('value', 3);
                        $('.calcLoan').html(Math.round(100));
                        var date = new Date();
                        var thisMidnight = new Date(date.getFullYear(), date.getMonth(), date.getDate(), 23, 59, 59);
                        $.cookie('clicked',true,{expires: thisMidnight});
                    } else {
                        console.log('Something went wrong during commnunicate with server');
                    }
                });
        });
     }
