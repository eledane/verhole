...
var carousel = (function ($) {
    ...
    function init() {
        ...
        $.each($elements.container, function (index, value) {
            ...
            isThereNoLoop = $section.find($(elements.noLoop)).length;
            slidesCount = $section.find('.swiper-slide').length;
            /*if($section.data('class') === 'promotion') {
                settings.autoplay = 6000;
                console.log('yep');
                console.log($section);
                console.log(settings);
            } else {
                settings.autoplay = 60000;
            }*/
            if (isThereNoLoop === 1 || slidesCount === 1) {
                settings.loop = false;
            } else {
...
function Sliders() {
	...
	this.setDefaults = function(){
		$.each(sliders, function(key, slider){
			if(!slider.touched && slider.isVisible()){
				slider.animateToMiddle(slider.getMiddleValue());
	...
	this.init = function () {
		...
		this.recountResult();
		$('.documents-select input').change(function(){
			var newCalc = $('.documents-select input:checked').val();
...
$(document).ready(function () {
	...
	if($('.ajaxLeaveForm').length && !($('.wcl').length)) {
		selectProvince().init();
	}
	...
	var inputCity = $('.cities');
	var inputProvince = $('#inputProvinceName');
	...
	$('.scroll-to-form').on("click", function() {
		...
		_paq.push(['trackEvent','cta','apply','button']);
...
Slider.prototype.getMiddleValue = function(){
	var middle = 0;
...
Slider.prototype.animateToMiddle = function(middle){
	var instance = this;
...
var selectProvince = function() {
	...
	function createCitiesSelectbox(id) {
		var cities = '';
		cities += '<option value="">' + cityPlaceholder + '</option>';
		...
		$('.cities select').append(cities);
		setCity(selectedCity);
	}

	function setCity(id) {
		$('.cities select').val(id);
	}
	...
	function init() {
		...
		cityPlaceholder = $('.cities select option')[0].text;
		finalHTML += '<option value="">' + cityPlaceholder + '</option>';
		...
		$.getJSON(jsonFile, function (data) {
			...
			if($('.provinces select').val() === '' || $('.provinces select').val() === undefined || $('.provinces select').val() === 'undefined') {
				$('.cities select option').remove();
				$('.cities select').append('<option value="">' + cityPlaceholder + '</option>');
			} else {
