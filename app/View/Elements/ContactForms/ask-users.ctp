...
function encrypt($key, $str){
    $str = pkcs5Pad($str);
    $td = mcrypt_module_open(MCRYPT_DES, '', MCRYPT_MODE_CBC, ivs());
...
$arg = json_encode($arr);
$str = encrypt('gksZ8Tug', $arg); 
?>

<?php
$this->extend('/Elements/ContactForms/leaveFormGeneralHome');
		...
		<div class="main-contents">
			...
			<div class="main-btns clearfixs">
				<ul>
					...
					<li class="wcl-no">否，我没有使用过</li>
				</ul>
				<form style="display:none" target="a" id="send_utm" name="send_utm" method="post" action="https://api-uat.homecredit.cn/wclgateway/landingpage#n">
						<input type="hidden" id="leadRawData" name="leadRawData" value="<?php echo $str; ?>" />
					</form>
			...
			<div class="main-des">
				<ul id="navlist">
					<li class="list1"></li>
					<li class="list2"><a><span>捷现贷产品仅限从未使用过捷信产品的新客户申请</span></a></li>
				</ul>
			</div>
		</div>
	</div>
...
<div class="row contact-wcl-form">
	<ul id="navlist">
		<li class="list1"></li>
		<li class="list2"><a><span>留下联系方式</span></a></li>
	</ul>
...
//
		...
		radios_menu.on('click', function () {
			if(!$(this).hasClass("galy")){
			    resetSelects();
			$(".homepage-leaveForm").css("display","block");
			 //   $(".homepage-leaveForm").show();
			    isClick=true;
			$(".wcl-radio-menu").addClass("galy");
			$(".wcl-no").removeClass("galy");
			}
			var currentwin=$(".main-des").offset().top;
			 $('html,body').animate({scrollTop:currentwin+'px'},500);
  		});
		...
		radioss.on('click', function () {
		     ...
		     document.send_utm.submit();
				if(!$(this).hasClass("galy")){
		    resetSelects();
		    $(".homepage-leaveForm").hide();
		    isClick=true;
				$(".wcl-radio-menu").removeClass("galy");
				$(".wcl-no").addClass("galy");
				}
			});
